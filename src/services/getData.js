...
export  async function getApiData(city) {
    const latLong = getCoords(city);

    const API_BASE = `https://api.openweathermap.org/data/2.5/onecall?lat=${latLong[0]}&lon=${latLong[1]}&exclude=minutely&appid=afdbaca5d0aef91b025609ff649b1f63&units=metric`;

    const data = await fetch (API_BASE);
    const json = await data.json.daily[0].temp.min;      
